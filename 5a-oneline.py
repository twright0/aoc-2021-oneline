...
print(reduce(
    lambda state, count: (state + (1 if count > 1 else 0)),
    reduce(
        lambda state, line: (state if (line[0][0] != line[1][0] and
                                   line[0][1] != line[1][1]) else (
        (x_range := (repeat(line[0][0]) if line[0][0] == line[1][0] else list(range(*(
            (line[0][0], line[1][0]+1) if line[0][0] < line[1][0] else (line[1][0], line[0][0]+1)
        ))))) and
        (y_range := (repeat(line[0][1]) if line[0][1] == line[1][1] else list(range(*(
            (line[0][1], line[1][1]+1) if line[0][1] < line[1][1] else (line[1][1], line[0][1]+1)
        ))))) and
        (reduce(
